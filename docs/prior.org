...
* Introduction
  ...
  bound. It involves the following:

  \[ E_q\left[\sum_{i,j,k} X_{ij} X_{ik} w_j w_k\right] = \sum_{i,j,k} X_{ij}
  X_{ik} E_q[w_j w_k] \]

  In the case of the spike-and-slab prior, this term goes to zero because
  \(Cov_q(w_j, w_k) = 0\). However, for our choice of prior it does not.

* Setup                                                            :noexport:

  #+NAME: srun
  #+BEGIN_SRC shell :dir (concat (file-name-as-directory (getenv "SCRATCH")) "spikeslab")
  sbatch --partition=gpu2 --time=4:00:00 --job-name=ipython3 --output=ipython3.out
  #!/bin/bash
...
* Case L = 1
  ...
  #+BEGIN_SRC ipython
    with tf.Graph().as_default():
      cov = empirical_cov(l=1, p=5, num_samples=50)
    cov
  #+END_SRC
